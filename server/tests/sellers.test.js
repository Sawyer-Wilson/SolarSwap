...
describe("POST /sellers/", () => {
  ...
  describe("when given valid seller parameters", () => {
    it("should save the new seller object in the database", async () => {
      await request(app).post(`/sellers`)
        .send(sellers[1]);
      const foundSeller = await Seller.findOne({ energyListingID: sellers[1].energyListingID });
      ...
      expect(foundSeller.lastName).toBe(sellers[1].lastName);
      expect(foundSeller.email).toBe(sellers[1].email);
    });

    it("should return the saved seller", async () => {
      const res = await request(app).post(`/sellers`)
        .send(sellers[1])
        .expect('Content-Type', /json/)
        .expect(200);
      const foundSeller = await Seller.findOne({ energyListingID: sellers[1].energyListingID });
      expect(res.body).toMatchObject(JSON.parse(JSON.stringify(foundSeller)));
    });
  });
  ...
  describe("when not given all required seller parameters", () => {
    it("should return 400 with an json error message", async () => {
      await request(app).post(`/sellers`)
        .send({
          firstName: "jane",
